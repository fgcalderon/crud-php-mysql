<?php 
	...
	if(isset($_GET['action'])) {
		...
		$statement = $dbmanager->prepare("DELETE FROM usuarios WHERE  id = :id");
		$statement->execute($data);


		if( $statement->rowCount() >= 1 ) {
			$mensaje =  "<div class='alert alert-danger'>El usuario ha sido eliminado</div>";
		} else {
		...
		//Codigos de Error https://docs.oracle.com/cd/E15817_01/appdev.111/b31228/appd.htm
	}
...
<div class="container margin-top-l">
	<div class="row">
		<div class="col-12">
			<h2 class="text-center">Eliminar Usuario (Delete)</h2>
			...
			<?php if($usuario != '') { ?>
					...
					<form action="usuario-delete.php" method="POST">
						<table class="table">
							<tr>
								<td>
									<input type="text" name="nombre_completo" value="<?php echo  $usuario["nombre_completo"] ?>" class="form-control">
								</td>
								<td>
									<input type="text" name="correo" value="<?php echo  $usuario["correo"] ?>" class="form-control">
								</td>
								<td>
									<input type="text" name="contrasena" value="<?php echo  $usuario["contrasena"] ?>" class="form-control">
								</td>
								<td>
